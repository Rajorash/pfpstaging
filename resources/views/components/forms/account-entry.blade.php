...
<x-ui.table-tbody>
    ...
    <tr class="border-transparent border-0 pt-2">
        <td class="pt-1 text-right">
            <label class="sm:w-1/3 pr-4 pl-4 pt-2 pb-2 mb-0 leading-normal text-right" for="amounts[{{$acc->id}}]">{{__($acc->name)}}</label>
        </td>
        <td class="pt-1 pr-24">
            <input class="block appearance-none w-full py-1 px-2 mb-1 text-base leading-normal bg-white text-gray-800 border border-gray-200 rounded" name="amounts[{{$acc->id}}]" type="text" value="{{old("amounts.$acc->id")}}">
        </td>
    </tr>
    @empty
