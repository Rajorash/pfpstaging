<div class="p-6 sm:px-20">
    <div class="grid gap-4 grid-cols-1 md:grid-cols-2">
        <div id="left_column">
            <div class="text-lg text-dark_gray2 leading-7 mb-4">{{__('Select and run a command')}}</div>
        ...
        <div id="right_column">
            <div class="text-lg text-dark_gray2 leading-7 mb-4">{{__('Result')}}</div>
            <div class="font-mono bg-black px-4 py-2 rounded
                ...
                @if($artisanResult['title'])
                    {!! $artisanResult['title'] !!}
                @else
                    <p>{{__('Select and run the command from the left listing. Waiting and enjoy after!')}}</p>
                @endif
            </div>
        </div>
