                    ...
                    <tr class="align-top">
                        ...
                        <x-ui.td>
                            <div class="">
                                ...
                                <div class="flex justify-between items-center pt-2  text-{{$flow->isNegative() ? 'red-500' : 'green-600' }}">
                                    {{ $flow->label }}
                                    <span class="inline-block text-right"><a href="/accounts/{{$acc->id}}/flow/{{$flow->id}}/edit" class="inline-block align-middle text-center select-none border font-normal whitespace-no-wrap rounded  no-underline py-1 px-2 leading-tight text-xs  bg-yellow-500 text-white hover:bg-yellow-600 mr-1">Edit Flow</a>
                                        <form class="inline" action="/accounts/{{$acc->id}}/flow/{{$flow->id}}" method="POST">
                                            @method('DELETE')
                                            @csrf
                                            <button type="submit" class="inline-block align-middle text-center select-none border font-normal whitespace-no-wrap rounded  no-underline py-1 px-2 leading-tight text-xs  bg-red-600 text-white hover:bg-red-700">Delete Flow</button>
                                        </form>
                                    </span>
                                </div>
                                @empty
